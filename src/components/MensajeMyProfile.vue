...
const quieroBuscar = async () => {
  if (variable.value.length >= 8) {
    ...
    try {
        ...
        // Verifica si existe content, sino usa datos directamente
...
const quieroRegistrar = async () => {
  try {
    ...
    tenemosDatos.value = true;
    quieroBuscar();
  } catch(_e) {
    errorRegPas.value = true;
    console.log("Error registrando pasajero");
  }
}
...
<template>
    <div>
        <h2 v-if="!fromDBPasajero && !quiereRegistrar">Para ver tu información introduce tu DNI.</h2>
        <h1 v-if="fromDBPasajero && !quiereRegistrar || tenemosDatos" class="green" id="titulo1">Bienvenid@ {{ fromDBPasajero.nombrePasajero}}</h1>
        <div v-if="!fromDBPasajero && !quiereRegistrar" class="div1">
          <InputText class="texto" v-model="variable" :invalid="variable.length < 8" placeholder="DNI" type="text" variant="filled" />
        </div>
            <div class="divTabla" v-if="fromDBPasajero && !quiereRegistrar || tenemosDatos" >
        ...
        <div class="div2">
            <Button v-if="!fromDBPasajero && !quiereRegistrar" @click="quieroBuscar"  label="Buscar" raised />
            <Button v-if="!fromDBPasajero && !quiereRegistrar" @click="registrar"  label="Registro" severity="secondary" raised />
            <Button v-if="!fromDBPasajero && quiereRegistrar" @click="quieroRegistrar"  label="Registrar" severity="secondary" raised />
        </div>
    </div>
</template>
